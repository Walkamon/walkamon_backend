...
CREATE TABLE pvp_step_sessions (
    ...
    row_version ROWVERSION NOT NULL,
    contract_version INT NOT NULL CONSTRAINT DF_pvp_step_sessions_contract_version DEFAULT 2,
    capture_metadata_json NVARCHAR(MAX) NULL,
    CONSTRAINT CK_pvp_step_sessions_purpose CHECK (purpose_code IN ('daily', 'pvp')),
    CONSTRAINT CK_pvp_step_sessions_sensor_mode CHECK (sensor_mode_code IN ('detector', 'counter', 'dual')),
...
CREATE TABLE step_sensor_batches (
    ...
    reconciliation_status VARCHAR(30) NOT NULL CONSTRAINT DF_step_sensor_batches_reconciliation_status DEFAULT 'unavailable',
    reconciliation_reason NVARCHAR(200) NULL,
    ...
    CONSTRAINT CK_step_sensor_batches_reconciliation_status CHECK (reconciliation_status IN ('unavailable', 'pending_reconciliation', 'accepted', 'suspicious', 'rejected')),
...
CREATE TABLE step_counter_evidence_samples (
    ...
    CONSTRAINT FK_step_counter_samples_batch FOREIGN KEY (batch_id)
        REFERENCES step_sensor_batches(step_sensor_batch_id) ON DELETE CASCADE
);
GO
...
CREATE TABLE validated_step_records (
    ...
    client_event_id UNIQUEIDENTIFIER NULL,
    boot_session_id UNIQUEIDENTIFIER NULL,
    sensor_elapsed_realtime_ns BIGINT NULL,
    ...
    CONSTRAINT CK_validated_step_records_status CHECK (validation_status IN ('accepted', 'pending', 'rejected', 'suspicious')),
...
GO
CREATE UNIQUE INDEX UX_step_counter_samples_batch_index
    ON step_counter_evidence_samples(batch_id, sample_index);
GO
CREATE UNIQUE INDEX UX_step_counter_samples_client_id
    ON step_counter_evidence_samples(client_sample_id);
GO
CREATE INDEX IX_step_counter_samples_boot_elapsed
    ON step_counter_evidence_samples(boot_session_id, sensor_elapsed_realtime_ns);
GO
...
GO
CREATE UNIQUE INDEX UX_validated_step_records_session_client_event
    ON validated_step_records(step_session_id, client_event_id)
