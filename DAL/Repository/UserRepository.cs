...
public class UserRepository : IUserRepository
{
    ...
    {
        return _context.UserProfiles.AnyAsync(
            profile => profile.NormalizedUsername == normalizedUsername
                && (!excludedUserId.HasValue || profile.UserId != excludedUserId.Value));
    }

    public Task<Role?> GetRoleByCodeAsync(string roleCode)
    {
        return _context.Roles.SingleOrDefaultAsync(role => role.RoleCode == roleCode);
    }
    ...
    {
        return _context.OtpRequests.CountAsync(
            otp => otp.PurposeCode == "verify_email"
        ...
        IEnumerable<string> settingKeys)
    {
        ...
        return await _context.SystemSettings
            ...
            .ToDictionaryAsync(setting => setting.SettingKey, setting => setting.SettingValue);
    }

    public async Task CleanupExpiredPendingRegistrationsAsync(DateTime createdBeforeUtc)
    {
        var users = await _context.Users
            .Where(user => user.StatusCode == "disabled"
                && !user.EmailConfirmed
        ...
        _context.UserProfiles.RemoveRange(
            users.Where(user => user.UserProfile != null)
